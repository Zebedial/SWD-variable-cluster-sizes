...
## prevalence and prevalence trend under pattern 
## 4 within-cluster imbalance, NEX true correlation
## (Web Tables 8, 9, 10, 11)
#############################################
...
# baseline prevalence and prevalence trend, when pattern 4 within-cluster imbalance is 
...
prev.table <- function(I, t, cmean, alpha0, alpha1, delta, type, case, nsims = 1000, 
                       seed = 8888) {
        ...
        RE.constant[i, j] <-
          sumVAR(n.equal(I, t, cmean),
                 ...
                 c(alpha0, alpha1)) / sumVAR(
                   n.variable.twoway(I, t, CV[i], cmean, case, p1 = 0.1),
                   scheme(I, t),
                   delta,
                   beta.constant,
                   c(alpha0, alpha1)
...
############################################################
## Scenario 1: I=24, pattern 4 within-cluster imbalance, alpha1=0.05
############################################################
...
write.csv(prev_out, file = paste0(dir, "/nuisance_nex_pattern4_1.csv"))

############################################################
## Scenario 2: I=24, pattern 4 within-cluster imbalance, alpha1=0.025
############################################################
...
write.csv(prev_out, file = paste0(dir, "/nuisance_nex_pattern4_2.csv"))

#############################################################
## Scenario 3: I=24, pattern 4 within-cluster imbalance, IND working 
##             correlation, alpha1=0.05
#############################################################
...
write.csv(prev_out, file = paste0(dir, "/nuisance_nex_pattern4_1_naive.csv"))

#############################################################
## Scenario 4: I=24, pattern 4 within-cluster imbalance, IND working 
##             correlation, alpha1=0.025
#############################################################
...
write.csv(prev_out, file = paste0(dir, "/nuisance_nex_pattern4_2_naive.csv"))


library(xtable)

xtable(read.csv(paste0(dir, "/nuisance_nex_pattern4_1.csv"), header = T))
xtable(read.csv(paste0(dir, "/nuisance_nex_pattern4_2.csv"), header = T))
xtable(read.csv(paste0(
  dir, "/nuisance_nex_pattern4_1_naive.csv"
), header = T))
xtable(read.csv(paste0(
  dir, "/nuisance_nex_pattern4_2_naive.csv"
), header = T))
