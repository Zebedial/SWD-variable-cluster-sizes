#####################################################################
# Illustration of Impact of number of period (J) under ED correlation 
# structure (Web Tables 12, 13, 14, 15)
#####################################################################
...
# values of CV, different patterns of within-cluster variability, 
# given other required parameters, when the true correlation model is 
...
#
# Output: 
# a table of minimum lower quartile (25 percentile), median, mean, 
# upper quartile (75 percentile), and maximum of RE as a function of 
# different CV and patterns of within-cluster imbalance
#######################################################################

CV_table.exp <- function(I, t, cmean,alpha0,rho, delta, base, end, type, 
      ...
      for (j in 1:nsims) {
        ...
        RE.0[i, j] <-
          sumVAR.exp(n.equal(I, t, cmean), scheme(I, t), delta, beta, 
                     alpha0, rho) /
          sumVAR.exp(n.variable(I, t, CV[i], cmean),
                     scheme(I, t),
                     delta,
                     beta,
                     alpha0,
        ...
        RE.1[i, j] <-
          sumVAR.exp(n.equal(I, t, cmean), scheme(I, t), delta, beta, 
                     alpha0, rho) /
          sumVAR.exp(
            n.variable.twoway(I, t, CV[i], cmean, case = 1, p1 = initial),
            scheme(I, t),
            delta,
            beta,
            alpha0,
        ...
        RE.2[i, j] <-
          sumVAR.exp(n.equal(I, t, cmean), scheme(I, t), delta, beta, 
                     alpha0, rho) /
          sumVAR.exp(
            n.variable.twoway(I, t, CV[i], cmean, case = 2, p1 = initial),
            scheme(I, t),
            delta,
            beta,
            alpha0,
        ...
        RE.3[i, j] <-
          sumVAR.exp(n.equal(I, t, cmean), scheme(I, t), delta, beta, 
                     alpha0, rho) /
          sumVAR.exp(
            n.variable.twoway(I, t, CV[i], cmean, case = 3, p1 = initial),
            scheme(I, t),
            delta,
            beta,
            alpha0,
        ...
        RE.4[i, j] <-
          sumVAR.exp(n.equal(I, t, cmean), scheme(I, t), delta, beta,
                     alpha0, rho) /
          sumVAR.exp(
            n.variable.twoway(I, t, CV[i], cmean, case = 4, p1 = initial),
            scheme(I, t),
            delta,
            beta,
            alpha0,
      ...
      for (j in 1:nsims) {
        ...
        RE.0[i, j] <-
          sumVAR.exp.naive(n.equal(I, t, cmean), scheme(I, t), delta, beta,
                           alpha0, rho) /
          sumVAR.exp.naive(n.variable(I, t, CV[i], cmean),
                           scheme(I, t),
                           delta,
                           beta,
                           alpha0,
...
setwd("C:/Users/ASUS/Desktop/YCAS/result tables/EXP_tables")
...
# scenarios with J=12, 24, 48, and 96 (under ED true correlation model)
####################################################################################


#############################################
## J and CV table (Scenario 1)
#############################################
...
J_CV_table_exp1 <- data.frame(cbind(WCor, J, CV.data.new))

write.csv(J_CV_table_exp1, "./J_CV_table_ed_12.csv")

#############################################
## J and CV table (Scenario 2)
#############################################
...
J_CV_table_exp2 <- data.frame(cbind(WCor, J, CV.data.new))

write.csv(J_CV_table_exp2, "./J_CV_table_ed_24.csv")


#############################################
## J and CV table (Scenario 3)
#############################################
...
J_CV_table_exp3 <- data.frame(cbind(WCor, J, CV.data.new))

write.csv(J_CV_table_exp3, "./J_CV_table_ed_48.csv")


#############################################
## J and CV table (Scenario 4)
#############################################
...
J_CV_table_exp4 <- data.frame(cbind(WCor, J, CV.data.new))

write.csv(J_CV_table_exp4, "./J_CV_table_ed_96.csv")