...
pdf("REvsCV_NEX_pattern2_1.pdf", width=10, height=7, 
    paper="special")


#CV vs RE

REvsCV1 <- read.csv(paste0(dir, "/REvsCV_1.2.1.csv"), header = T)
REvsCV2 <- read.csv(paste0(dir, "/REvsCV_1.2.4.csv"), header = T)
REvsCV1.naive <-
  read.csv(paste0(dir, "/REvsCV.naive_1.2.1.csv"), header = T)
REvsCV2.naive <-
  read.csv(paste0(dir, "/REvsCV.naive_1.2.4.csv"), header = T)
