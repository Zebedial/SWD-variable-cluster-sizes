...
pdf("REvsCV_NEX_pattern3_1.pdf", width=10, height=7, 
    paper="special")

#CV vs RE

REvsCV1 <- read.csv(paste0(dir, "/REvsCV_1.3.1.csv"), header = T)
REvsCV2 <- read.csv(paste0(dir, "/REvsCV_1.3.4.csv"), header = T)
REvsCV1.naive <-
  read.csv(paste0(dir, "/REvsCV.naive_1.3.1.csv"), header = T)
REvsCV2.naive <-
  read.csv(paste0(dir, "/REvsCV.naive_1.3.4.csv"), header = T)
