...
## prevalence and prevalence trend under no 
## within-cluster imbalance, ED true correlation
## (Web Tables 16, 17, 18, 19)
#############################################
...
# baseline prevalence and prevalence trend, when no within-cluster imbalance is introduced.
# The output is a subset of the final table.
#
# Input:
...
prev.table <- function(I, t, cmean, alpha0, rho, delta, type, nsims = 1000, 
                       seed = 8888) {
      ...
      for (j in 1:nsims) {
        ...
        RE.constant[i, j] <-
          sumVAR.exp(n.equal(I, t, cmean),
                     ...
                     beta.constant,
                     alpha0,
                     rho) / sumVAR.exp(n.variable(I, t, CV[i], cmean),
                                       scheme(I, t),
                                       delta,
                                       beta.constant,
                                       alpha0,
      ...
      for (j in 1:nsims) {
        ...
        RE.constant[i, j] <-
          sumVAR.exp.naive(n.equal(I, t, cmean),
                           ...
                           beta.constant,
                           alpha0,
                           rho) / sumVAR.exp.naive(n.variable(I, t, CV[i], 
                                                              cmean),
                                                   scheme(I, t),
                                                   delta,
                                                   beta.constant,
...
############################################################
## Scenario 1: I=24, no within-cluster imbalance, rho=0.9
############################################################
...
write.csv(prev_out, file = paste0(dir, "/nuisance_ed_fixed_1.csv"))

############################################################
## Scenario 2: I=24, no within-cluster imbalance, rho=0.5
############################################################
...
write.csv(prev_out, file = paste0(dir, "/nuisance_ed_fixed_2.csv"))

#############################################################
## Scenario 3: I=24, no within-cluster imbalance, IND working correlation,
##             rho=0.9
#############################################################
...
write.csv(prev_out, file = paste0(dir, "/nuisance_ed_fixed_1_naive.csv"))

#############################################################
## Scenario 4: I=24, no within-cluster imbalance, IND working correlation,
##             rho=0.5
#############################################################
...
write.csv(prev_out, file = paste0(dir, "/nuisance_ed_fixed_2_naive.csv"))


library(xtable)

xtable(read.csv(paste0(dir, "/nuisance_ed_fixed_1.csv"), header = T))
xtable(read.csv(paste0(dir, "/nuisance_ed_fixed_2.csv"), header = T))
xtable(read.csv(paste0(dir, "/nuisance_ed_fixed_1_naive.csv"), header =
                  T))
xtable(read.csv(paste0(dir, "/nuisance_ed_fixed_2_naive.csv"), header =
                  T))
