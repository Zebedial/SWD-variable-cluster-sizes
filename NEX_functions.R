...
# Structure is Nested Exchangeable (NEX)
############################################################################
...
# (NEX) correlation structure by an independence (IND) correlation structure
#
# Input:
# n: a (number of clusters) x (number of periods) size matrix of cluster 
#    period sizes
...
sumVAR.naive <- function(n, scheme, delta, beta, alpha) {
  t <- ncol(n)
  I <- nrow(n)
  trtSeq <- matrix(0, t - 1, t)
  trtSeq[upper.tri(trtSeq)] <- 1
