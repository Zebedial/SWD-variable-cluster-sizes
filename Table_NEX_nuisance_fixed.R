#############################################
## Sensitivity to treatment effect, baseline 
## prevalence and prevalence trend under no 
## within-cluster imbalance, NEX true correlation
## (Web Tables 4, 5, 6, 7)
#############################################

dir="C:\\Users\\ASUS\\Desktop\\YCAS\\result tables\\NEX_tables"
#setwd("C:\\Users\\ASUS\\Desktop\\YCAS\\Simulation\\Nest Exchangeable Correlation")
source("./NEX_functions.R")


####################################################################################
# Construct datasets used to produce the summary table of the impact of treatment effect
# baseline prevalence and prevalence trend, when no within-cluster imbalance is introduced.
# The output is a subset of the final table.
#
# Input:
# I: number of clusters (I)
# t: number of periods (J)
# cmean: mean cluster-period size
# alpha0: WP-ICC 
# alpha1: BP-ICC
# delta: treatment effect in scale of log(OR)
# type: working correlation structure. 1 for correctly specified working correlation 
#       structure, 2 for independence working correlation structure
# seed: a seed to control reproducible output with default value 8888
# nsims: times for the simulation runs with default value 1000
#
# Output: a table with minimum, lower quartile (25 percentile), median, mean, upper
#         quartile (75 percentile), and maximum of RE as a function of CV, baseline
#         prevalence, and prevalence trend specified in section 4 of the main article
#         given a set of other required parameters
####################################################################################


prev.table <- function(I, t, cmean, alpha0, alpha1, delta, type, nsims = 1000, 
                       seed = 8888) {
  
  CV <- rep(c(0.25, 0.75, 1.25), each = 2)
  Base <- rep(c(0.1, 0.3), 3)
  
  RE.constant <- matrix(NA, 6, nsims)
  RE.increase <- matrix(NA, 6, nsims)
  RE.decrease <- matrix(NA, 6, nsims)
  
  for (i in 1:6) {
    base <- Base[i]
    end.constant <- Base[i]
    end.increase <- Base[i] * 1.2
    end.decrease <- Base[i] * 0.8
    
    p1 <- seq(base, end.constant, length.out = t)
    beta.constant <- log(p1 / (1 - p1))
    p2 <- seq(base, end.increase, length.out = t)
    beta.increase <- log(p2 / (1 - p2))
    p3 <- seq(base, end.decrease, length.out = t)
    beta.decrease <- log(p3 / (1 - p3))
    
    if (type == 1) {
      for (j in 1:nsims) {
        set.seed(seed + j)
        RE.constant[i, j] <-
          sumVAR(n.equal(I, t, cmean),
                 scheme(I, t),
                 delta,
                 beta.constant,
                 c(alpha0, alpha1)) / sumVAR(
                   n.variable(I, t, CV[i], cmean),
                   scheme(I, t),
                   delta,
                   beta.constant,
                   c(alpha0, alpha1)
                 )
        RE.increase[i, j] <-
          sumVAR(n.equal(I, t, cmean),
                 scheme(I, t),
                 delta,
                 beta.increase,
                 c(alpha0, alpha1)) / sumVAR(
                   n.variable(I, t, CV[i], cmean),
                   scheme(I, t),
                   delta,
                   beta.increase,
                   c(alpha0, alpha1)
                 )
        RE.decrease[i, j] <-
          sumVAR(n.equal(I, t, cmean),
                 scheme(I, t),
                 delta,
                 beta.decrease,
                 c(alpha0, alpha1)) / sumVAR(
                   n.variable(I, t, CV[i], cmean),
                   scheme(I, t),
                   delta,
                   beta.decrease,
                   c(alpha0, alpha1)
                 )
      }
    }
    
    else if (type == 2) {
      for (j in 1:nsims) {
        set.seed(seed + j)
        RE.constant[i, j] <-
          sumVAR.naive(n.equal(I, t, cmean),
                       scheme(I, t),
                       delta,
                       beta.constant,
                       c(alpha0, alpha1)) / sumVAR.naive(
                         n.variable(I, t, CV[i], cmean),
                         scheme(I, t),
                         delta,
                         beta.constant,
                         c(alpha0, alpha1)
                       )
        RE.increase[i, j] <-
          sumVAR.naive(n.equal(I, t, cmean),
                       scheme(I, t),
                       delta,
                       beta.increase,
                       c(alpha0, alpha1)) / sumVAR.naive(
                         n.variable(I, t, CV[i], cmean),
                         scheme(I, t),
                         delta,
                         beta.increase,
                         c(alpha0, alpha1)
                       )
        RE.decrease[i, j] <-
          sumVAR.naive(n.equal(I, t, cmean),
                       scheme(I, t),
                       delta,
                       beta.decrease,
                       c(alpha0, alpha1)) / sumVAR.naive(
                         n.variable(I, t, CV[i], cmean),
                         scheme(I, t),
                         delta,
                         beta.decrease,
                         c(alpha0, alpha1)
                       )
      }
    }
  }
  
  RE.constant_min <- apply(RE.constant, 1, quantile)[1, ]
  RE.constant_25 <- apply(RE.constant, 1, quantile)[2, ]
  RE.constant_median <- apply(RE.constant, 1, quantile)[3, ]
  RE.constant_75 <- apply(RE.constant, 1, quantile)[4, ]
  RE.constant_max <- apply(RE.constant, 1, quantile)[5, ]
  RE.constant_mean <- apply(RE.constant, 1, mean)
  
  RE.increase_min <- apply(RE.increase, 1, quantile)[1, ]
  RE.increase_25 <- apply(RE.increase, 1, quantile)[2, ]
  RE.increase_median <- apply(RE.increase, 1, quantile)[3, ]
  RE.increase_75 <- apply(RE.increase, 1, quantile)[4, ]
  RE.increase_max <- apply(RE.increase, 1, quantile)[5, ]
  RE.increase_mean <- apply(RE.increase, 1, mean)
  
  RE.decrease_min <- apply(RE.decrease, 1, quantile)[1, ]
  RE.decrease_25 <- apply(RE.decrease, 1, quantile)[2, ]
  RE.decrease_median <- apply(RE.decrease, 1, quantile)[3, ]
  RE.decrease_75 <- apply(RE.decrease, 1, quantile)[4, ]
  RE.decrease_max <- apply(RE.decrease, 1, quantile)[5, ]
  RE.decrease_mean <- apply(RE.decrease, 1, mean)
  
  Prevalence_compare <-
    data.frame(
      cbind(
        CV,
        Base,
        RE.constant_median,
        RE.constant_25,
        RE.constant_75,
        RE.increase_median,
        RE.increase_25,
        RE.increase_75,
        RE.decrease_median,
        RE.decrease_25,
        RE.decrease_75,
        RE.constant_mean,
        RE.constant_min,
        RE.constant_max,
        RE.increase_mean,
        RE.increase_min,
        RE.increase_max,
        RE.decrease_mean,
        RE.decrease_min,
        RE.decrease_max
      )
    )
  return(Prevalence_compare)
}

############################################################
## Scenario 1: I=24, no within-cluster imbalance, alpha1=0.05
############################################################

treatment <- rep(c("log(0.35)", "log(0.75)"), each = 6)
prevtable <-
  rbind(
    prev.table(
      I = 24,
      t = 5,
      cmean = 100,
      alpha0 = 0.05,
      alpha1 = 0.05,
      delta = log(0.35),
      type = 1
    ),
    prev.table(
      I = 24,
      t = 5,
      cmean = 100,
      alpha0 = 0.05,
      alpha1 = 0.05,
      delta = log(0.75),
      type = 1
    )
  )
prevtable <- cbind(treatment, prevtable)

prev_out1 <- prevtable[, 1:3]
prev_out2 <- matrix(0, ncol = 3, nrow = 12)

for (i in 1:12) {
  for (j in 1:3) {
    prev_out2[i, j] <-
      paste0(round(prevtable[i, 3 * j + 1], 3),
             " (",
             round(prevtable[i, j * 3 + 2], 3),
             ", ",
             round(prevtable[i, j * 3 + 3], 3),
             ")")
  }
}
prev_out <- cbind(prev_out1, prev_out2)
colnames(prev_out) <-
  c("Treatment",
    "CV",
    "Baseline",
    "Constant",
    "Increaisng",
    "Decreasing")

write.csv(prev_out, file = paste0(dir, "/nuisance_nex_fixed_1.csv"))


############################################################
## Scenario 2: I=24, no within-cluster imbalance, alpha1=0.025
############################################################

treatment <- rep(c("log(0.35)", "log(0.75)"), each = 6)
prevtable <-
  rbind(
    prev.table(
      I = 24,
      t = 5,
      cmean = 100,
      alpha0 = 0.05,
      alpha1 = 0.025,
      delta = log(0.35),
      type = 1
    ),
    prev.table(
      I = 24,
      t = 5,
      cmean = 100,
      alpha0 = 0.05,
      alpha1 = 0.025,
      delta = log(0.75),
      type = 1
    )
  )
prevtable <- cbind(treatment, prevtable)

prev_out1 <- prevtable[, 1:3]
prev_out2 <- matrix(0, ncol = 3, nrow = 12)

for (i in 1:12) {
  for (j in 1:3) {
    prev_out2[i, j] <-
      paste0(round(prevtable[i, 3 * j + 1], 3),
             " (",
             round(prevtable[i, j * 3 + 2], 3),
             ", ",
             round(prevtable[i, j * 3 + 3], 3),
             ")")
  }
}
prev_out <- cbind(prev_out1, prev_out2)
colnames(prev_out) <-
  c("Treatment",
    "CV",
    "Baseline",
    "Constant",
    "Increaisng",
    "Decreasing")

write.csv(prev_out, file = paste0(dir, "/nuisance_nex_fixed_2.csv"))

#############################################################
## Scenario 3: I=24, no within-cluster imbalance, IND working correlation,
##             alpha1=0.05
#############################################################

treatment <- rep(c("log(0.35)", "log(0.75)"), each = 6)
prevtable <-
  rbind(
    prev.table(
      I = 24,
      t = 5,
      cmean = 100,
      alpha0 = 0.05,
      alpha1 = 0.05,
      delta = log(0.35),
      type = 2
    ),
    prev.table(
      I = 24,
      t = 5,
      cmean = 100,
      alpha0 = 0.05,
      alpha1 = 0.05,
      delta = log(0.75),
      type = 2
    )
  )
prevtable <- cbind(treatment, prevtable)

prev_out1 <- prevtable[, 1:3]
prev_out2 <- matrix(0, ncol = 3, nrow = 12)

for (i in 1:12) {
  for (j in 1:3) {
    prev_out2[i, j] <-
      paste0(round(prevtable[i, 3 * j + 1], 3),
             " (",
             round(prevtable[i, j * 3 + 2], 3),
             ", ",
             round(prevtable[i, j * 3 + 3], 3),
             ")")
  }
}
prev_out <- cbind(prev_out1, prev_out2)
colnames(prev_out) <-
  c("Treatment",
    "CV",
    "Baseline",
    "Constant",
    "Increaisng",
    "Decreasing")

write.csv(prev_out, file = paste0(dir, "/nuisance_nex_fixed_1_naive.csv"))

#############################################################
## Scenario 4: I=24, no within-cluster imbalance, IND working correlation,
##             alpha1=0.025
#############################################################

treatment <- rep(c("log(0.35)", "log(0.75)"), each = 6)
prevtable <-
  rbind(
    prev.table(
      I = 24,
      t = 5,
      cmean = 100,
      alpha0 = 0.05,
      alpha1 = 0.025,
      delta = log(0.35),
      type = 2
    ),
    prev.table(
      I = 24,
      t = 5,
      cmean = 100,
      alpha0 = 0.05,
      alpha1 = 0.025,
      delta = log(0.75),
      type = 2
    )
  )
prevtable <- cbind(treatment, prevtable)

prev_out1 <- prevtable[, 1:3]
prev_out2 <- matrix(0, ncol = 3, nrow = 12)

for (i in 1:12) {
  for (j in 1:3) {
    prev_out2[i, j] <-
      paste0(round(prevtable[i, 3 * j + 1], 3),
             " (",
             round(prevtable[i, j * 3 + 2], 3),
             ", ",
             round(prevtable[i, j * 3 + 3], 3),
             ")")
  }
}
prev_out <- cbind(prev_out1, prev_out2)
colnames(prev_out) <-
  c("Treatment",
    "CV",
    "Baseline",
    "Constant",
    "Increaisng",
    "Decreasing")

write.csv(prev_out, file = paste0(dir, "/nuisance_nex_fixed_2_naive.csv"))


library(xtable)

xtable(read.csv(paste0(dir, "/nuisance_nex_fixed_1.csv"), header = T))
xtable(read.csv(paste0(dir, "/nuisance_nex_fixed_2.csv"), header = T))
xtable(read.csv(paste0(dir, "/nuisance_nex_fixed_1_naive.csv"), header =
                  T))
xtable(read.csv(paste0(dir, "/nuisance_nex_fixed_2_naive.csv"), header =
                  T))


